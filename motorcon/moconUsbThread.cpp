...
void UsbThread::threadInitFunction()
{
   printf("moconUsbThread::threadInitFunction\n");
...
void UsbThread::threadRunFunction()
{
restart:
   // Guard.
   ...
   while (!BaseClass::mTerminateFlag)
   {
      ...
      tRet = read(mPortFd, mUsbCom.mRxBuffer, 200);
      if (tRet < 0)
      {
      ...
      mUsbCom.mRxLength = tRet;
      mUsbCom.doProcess();
      Prn::print(Prn::Show1, "Emulator Tx %d", mUsbCom.mTxLength);

      // Write a response string.
      if (mUsbCom.mTxLength)
      {
         // Write a response string.
         tRet = write(mPortFd, mUsbCom.mTxBuffer, mUsbCom.mTxLength);
         if (tRet < 0)
         {
...
void UsbThread::threadExitFunction()
{
   printf("moconUsbThread::threadExitFunction\n");
}
...
void UsbThread::shutdownThread()
{
   printf("moconUsbThread::shutdownThread\n");
...
void UsbThread::sendString(const char* aString)
{
   // Guard.
   if (mPortFd < 0) return;
